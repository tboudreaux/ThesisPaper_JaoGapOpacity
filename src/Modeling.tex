...
\textbf{Despite the alternative view of convection provided by
\citet{MacDonald2018} disscused in Section \ref{sec:JaoGap} and given that the
mixing timescales in these low mass stars are so short \citep[$\sim10^{4}$s over a
single model shell per][]{Kapyla2021} that instantaneous mixing is a valid approximation, we
elect to use the instantaneous mixing model of convection. Additionally, we
include gravitational settling in our models, where
...
\begin{figure*}
	\centering
	\includegraphics[width=0.85\textwidth]{OPALOPLIB_popsynth_compare.pdf}
...
\textbf{In order to test the sensitivity of Gap properties to mixing length we
evolve three seperate sets OPLIB of models. The first (OPLIB-A) uses a GS98
solar calibrated mixing length, the second (OPLIB-B) uses a mixing length of
1.5, and the third (OPLIB-C) uses a mixing length of 1.0.}

\textbf{We find a clear inverse corelation between mixing length parameter used
and the magnitude of the Jao Gap Figures \ref{fig:MixingLengthCMD} \&
\ref{fig:MixingLengthScaling} ($\mu_{G} \propto -1.5\alpha_{ML}$, where
$\mu_{G}$ is the mean magnitude of the Gap). This is somewhat suprising given
the long estabilished view that the mixing length parameter is of little
...
The mixing length then provides a free parameter which may be used to shift the
gap location in order to better match observations without having a major
impact on the effective temperature of models. Moreover, recent work indicates
that using a solar calibrated mixing length is not appropriate for all stars
...
\begin{figure}
	...
	\caption{\textbf{Location of the two identified paucity of stars in OPLIB synthetic
	populations as a function of the mixing length used.}}
	\label{fig:MixingLengthScaling}
\end{figure}

\textbf{Given the variability of gap location with mixing length, it is
possible that a better fit to the gap location may be achived through
adjustment of the convective mixing length parameter. However, calibrations of
the mixing length for stars other than the sun have focused on stars with
effective temperature at or above that of the sun and there are no current
calibrations of the mixing length parameter for M dwarfs. Moreover, there are
additional uncertanties when comparing the predicted gap location to the
...
if the mixing length is to be used as a gap location free paremerter. Given the
dangers of freely adjustable paremerters and the lack of an a priori
expectation for what the convective mixing parameter should be for the
population of M Dwarfs in the Gaia DR2 and EDR3 CMD we caution against
adjusting the mixing length to force the Jao Gap magnitude into better
